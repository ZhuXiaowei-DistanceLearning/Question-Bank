...
import com.zxw.thread.batch.TaskDispatcher;
import com.zxw.thread.batch.TaskDispatchers;
import com.zxw.thread.batch.TaskProcessor;
...
public class TaskDispatchersTest {
    ...
    private TaskDispatcher<Integer, TaskProcessor.ProcessingResult> dispatcher;
    ...
    public void testSingleTaskDispatcher() throws Exception {
        ...
        dispatcher.process(1, TaskProcessor.ProcessingResult.Success, System.currentTimeMillis() + 60 * 1000);
        TaskProcessor.ProcessingResult result = processor.completedTasks.poll(5, TimeUnit.SECONDS);
        assertThat(result, is(equalTo(TaskProcessor.ProcessingResult.Success)));
    }
    ...
    public void testBatchingDispatcher() throws Exception {
        ...
        dispatcher.process(1, TaskProcessor.ProcessingResult.Success, System.currentTimeMillis() + 60 * 1000);
        dispatcher.process(2, TaskProcessor.ProcessingResult.Success, System.currentTimeMillis() + 60 * 1000);
